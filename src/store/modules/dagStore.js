...
const dagStore = {
  ...
  mutations: {
    ...
    ACTIVE_DATA: (state, action) => {
      ...
      const typeToText = {
        running: ['节点激活', 'warning', '激活', 'active'],
        finished: ['训练完成', 'success', '完成', 'success']
      }
      action.map((item, t) => {
        // 节点的动作
        nodes.forEach(each => {
          if (each.id === item.node_id) {
            ...
            each.name = item.node_name
            window.setTimeout(() => {
              notice({
                title: typeToText[item.type][0],
                type: typeToText[item.type][1],
                message: `${item.node_name}节点已${typeToText[item.type][2]}`
              })
            }, t * 200)
          }
        })
    ...
    STOP_DATA: state => {
      ...
      nodes.forEach(item => {
        item.type === 'active' ? (item.type = 'pause') : ''
      })
      edges.forEach(item => {
        item.type === 'active' ? (item.type = 'pause') : ''
      })
